...
def _wants_visible_browser(params: dict) -> bool:
    """True only when the caller explicitly requests a visible browser window."""
    if params.get("visible") is True:
        return True
...
def _session_key(browser_name: str, *, headless: bool) -> str:
    mode = "headless" if headless else "visible"
    return f"{browser_name}::{mode}"
...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".emily_profiles" / browser
...
class _BrowserSession:
    """
    Playwright session for one browser. Headless by default (background automation).
    Visible mode uses the user's real profile via launch_persistent_context.
    """

    def __init__(self, browser_name: str, *, headless: bool = True):
    ...
    async def _launch_headless(self):
        ...
        print(f"[Browser] ✅ Headless {self.browser_name} launched")
    ...
    async def _launch_headed(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".emily_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using E.M.I.L.Y. profile")
                emily = str(Path.home() / ".emily_profiles" / "firefox_emily")
                Path(emily).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(emily, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".emily_profiles" / "safari")
        ...
        emily_profile = str(Path.home() / ".emily_profiles" / self.browser_name)
        Path(emily_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with E.M.I.L.Y. profile: {emily_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(emily_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with E.M.I.L.Y. profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "emily_screenshot.png")
...
class _SessionRegistry:
    ...
    def _get_or_create(self, browser_name: str, *, headless: bool = True) -> _BrowserSession:
        key = _session_key(browser_name, headless=headless)
        with self._lock:
            if key not in self._sessions:
                ...
                mode = "headless" if headless else "visible"
                print(f"[Registry] New session: {browser_name} ({mode})")
    ...
    def switch(self, browser_name: str) -> str:
        browser_name = _ALIASES.get(browser_name.lower().strip(), browser_name.lower().strip())
        self._get_or_create(browser_name, headless=True)
        self._active_browser = browser_name
    ...
    def close_one(self, browser_name: str) -> str:
        ...
        closed = []
        with self._lock:
            keys = [k for k in self._sessions if k == browser_name or k.startswith(prefix)]
            sessions = [self._sessions.pop(k) for k in keys]
        for sess in sessions:
            try:
                sess.close()
                closed.append(sess.browser_name)
            except Exception:
                pass
        if closed:
            if self._active_browser == browser_name:
                self._active_browser = ""
            unique = sorted(set(closed))
            return f"{', '.join(unique)} closed."
    ...
    def list_sessions(self) -> str:
        with self._lock:
            ...
            for key in self._sessions:
                browser_name, _, mode = key.partition("::")
                marker = " ◀ active" if browser_name == self._active_browser else ""
                lines.append(f"  • {browser_name} ({mode}){marker}")
...
def browser_control(
    ...
    headless = not _wants_visible_browser(params)

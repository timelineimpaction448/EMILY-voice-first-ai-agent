...
from core.llm.helpers import LegacyLLMModel, llm_multimodal


def _get_gemini(model: str = GEMINI_MODEL):
    return LegacyLLMModel()
...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    return DESKTOP / f"emily_code{ext}"
...
def _take_screenshot() -> Path | None:
    try:
        import pyautogui
        screenshot_path = Path.home() / "Desktop" / f"emily_debug_{int(time.time())}.png"
...
Be specific and actionable. If you see an error message, quote it exactly."""
        ...
        analysis = llm_multimodal(analysis_prompt, image_bytes, mime_type="image/png").strip()
